...
const ERP_LOG_COLUMNS = [
  { key: 'time',        label: '시간',     width: 150, sortable: true,
    render: l => <span style={{ fontFamily: 'monospace' }}>{l.time}</span> },
  { key: 'bizNo',       label: '사업자번호', width: 120, sortable: true },
  { key: 'companyName', label: '사업장명', width: 150, sortable: true,
    render: l => <span style={{ fontWeight: 600 }}>{l.companyName}</span> },
  { key: 'task',        label: '업무',     width: 90,  sortable: true, render: l => <Badge value={l.task} /> },
  { key: 'menu',        label: '메뉴',     width: 120, sortable: true },
  { key: 'user',        label: '사용자',   width: 90,  sortable: true },
  { key: 'action',      label: '액션',     width: 90,  sortable: true },
  { key: 'duration',    label: '소요시간', width: 90,  sortable: true,
    render: l => <span style={{ fontFamily: 'monospace' }}>{l.duration}</span> },
];
    ...
    <div>
      ...
      <div className="kpi-grid">
        <div className="kpi-card">
          <div className="label">오늘 총 클릭</div>
          <div className="value">12,841</div>
        </div>
        <div className="kpi-card">
          <div className="label">활성 사업장(오늘)</div>
          <div className="value">438</div>
        </div>
        <div className="kpi-card">
          <div className="label">최다 사용 메뉴</div>
          <div className="value" style={{ fontSize: 16 }}>전표 입력</div>
        </div>
        <div className="kpi-card">
          <div className="label">미사용(30일)</div>
          <div className="value" style={{ color: 'var(--amber-600)' }}>87</div>
        </div>
      </div>
            ...
            <tbody>
              {BIZ_ACTIVITY.map((b, i) => (
                <tr key={i}>
                  ...
                  <td>{b.lastUse}</td>
                  <td>
                    <Badge value={b.activity} variant={ACTIVITY_VARIANT[b.activity]} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="card" style={{ padding: 0 }}>
        ...
        <div className="table-wrap" style={{ border: 'none', borderRadius: 0 }}>
          <ResizableTable columns={ERP_LOG_COLUMNS} data={filtered} emptyText="검색 결과가 없습니다." />
        </div>
      </div>
    </div>
  );
