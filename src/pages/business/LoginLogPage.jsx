...
export default function LoginLogPage() {
  ...
  function handleReset() {
    setPeriodType('기간별'); setSingleDate(''); setFromDate(''); setToDate('');
    setYear('2025'); setMonth('03'); setSearchTarget('사업장명'); setSearchKw('');
  ...
  return (
    <div>
      ...
      <div className="kpi-grid">
        <div className="kpi-card"><div className="label">조회기간 총 로그인</div><div className="value">8,742</div></div>
        <div className="kpi-card"><div className="label">사업장 수</div><div className="value">438</div></div>
        <div className="kpi-card"><div className="label">로그인 실패</div><div className="value" style={{ color: 'var(--amber-600)' }}>14</div></div>
        <div className="kpi-card"><div className="label">이상 탐지</div><div className="value" style={{ color: 'var(--red-600)' }}>2</div></div>
      </div>
      ...
      <div className="summary-bar">
        <div className="item"><span>조회결과</span><span className="val">{filtered.length}건</span><span style={{ color: 'var(--gray-400)' }}>(사업장 기준)</span></div>
        <div style={{ color: 'var(--purple-200)' }}>|</div>
        <div className="item"><span>총 로그인 횟수</span><span className="val">{totalLogin.toLocaleString()}회</span></div>
        <div style={{ color: 'var(--purple-200)' }}>|</div>
        <div className="item"><span>실패</span><span className="val" style={{ color: 'var(--red-600)' }}>{failCount}회</span></div>
      </div>
          ...
          <tbody>
            {filtered.map((l, i) => (
              <tr key={i}>
                ...
                <td style={{ textAlign: 'right', fontWeight: 700 }}>{l.loginCount.toLocaleString()}</td>
                <td>{l.lastLogin}</td>
              </tr>
            ))}
          </tbody>
